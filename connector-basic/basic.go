...
import (
	...
	"github.com/codenomdev/viona-plugins/connector-basic/i18n"
...
func init() {
	plugins.Register(&Connector{
		Config: &ConnectorConfig{},
	})
}

func (g *Connector) Info() plugins.Info {
	info := &util.Info{}
	info.GetInfo(Info)

	return plugins.Info{
		Name:        plugins.MakeTranslator(i18n.InfoName),
		SlugName:    info.SlugName,
		Description: plugins.MakeTranslator(i18n.InfoDescription),
		Author:      info.Author,
...
func (g *Connector) ConnectorName() plugins.Translator {
	if len(g.Config.Name) > 0 {
		return plugins.MakeTranslator(g.Config.Name)
	}
	return plugins.MakeTranslator(i18n.ConnectorName)
}
...
func (g *Connector) ConnectorReceiver(ctx plugins.EchoContext, receiverURL string) (userInfo plugins.ExternalLoginUserInfo, err error) {
	...
	userInfo = plugins.ExternalLoginUserInfo{
...
func (g *Connector) ConfigFields() []plugins.ConfigField {
	fields := make([]plugins.ConfigField, 0)
	...
	fields = append(fields, plugins.ConfigField{
		Name:  "check_email_verified",
		Type:  plugins.ConfigTypeSwitch,
		Title: plugins.MakeTranslator(i18n.ConfigCheckEmailVerifiedTitle),
		Value: g.Config.CheckEmailVerified,
		UIOptions: plugins.ConfigFieldUIOptions{
			Label: plugins.MakeTranslator(i18n.ConfigCheckEmailVerifiedLabel),
		},
	})
...
func createTextInput(name, title, desc, value string, require bool) plugins.ConfigField {
	return plugins.ConfigField{
		Name:        name,
		Type:        plugins.ConfigTypeInput,
		Title:       plugins.MakeTranslator(title),
		Description: plugins.MakeTranslator(desc),
		Required:    require,
		UIOptions: plugins.ConfigFieldUIOptions{
			InputType: plugins.InputTypeText,
		},
		Value: value,
	}
}
